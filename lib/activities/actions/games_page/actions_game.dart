...
class _ActionsGameState extends State<ActionsGame> {
  ...
  void _assignFromQueue() {
    if (actionQueue.isEmpty || queueIndex >= actionQueue.length) {
      actionQueue = List.from(actions)..shuffle(random);
      queueIndex = 0;
    }
    ...
    if (widget.selectedAction == "Shuffle" && gamesPlayedCount % 3 == 0) {
      ...
      if (shuffleWordList.isNotEmpty) {
        ...
        _assignFromQueue();
              ...
              widget.selectedAction != "Shuffle" && 
              !randomize) {
      // Use specific selected actions
  ...
  void setOneWrongNumber(){
    ...
    wrongActions = [actions[randomNumber2].toLowerCase()];
  }

  void setTwoWrongNumbers(){
    randomNumber2 = random.nextInt(actions.length);
    randomNumber3 = random.nextInt(actions.length);
    ...
    wrongActions = [actions[randomNumber2].toLowerCase(), actions[randomNumber3].toLowerCase()];
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (totalSteps % 2 == 1 && totalSteps <= 10) {
      currentActivity = TapComponent(onCompleted: nextStep, assetLink: actions[randomNumber].toLowerCase(), mainData: actions[correctIndex], onCorrectAction: triggerCorrectFlash, totalSteps: totalSteps, directory: "assets/actions_images/", objectVariation: "Action",);
    } 
    else if (totalSteps % 2 == 0 && totalSteps <= 10) {
      currentActivity = DragDropComponent(onCompleted: nextStep, assetLink: actions[randomNumber].toLowerCase(), mainData: actions[correctIndex], onCorrectAction: triggerCorrectFlash, totalSteps: totalSteps, directory: "assets/actions_images/", objectVariation: "Action",);
    } 
    else if(totalSteps % 2 == 1 && totalSteps >= 10){
      currentActivity = TapMultipleObjectsComponent(onCompleted: nextStep, correctAssetLink: actions[correctIndex].toLowerCase(), wrongAssetLinks: wrongActions, mainData: actions[correctIndex], onCorrectAction: triggerCorrectFlash, onIncorrectAction: triggerIncorrectFlash, directory: "assets/actions_images/", objectVariation: "Action",);
    }
    else {
      currentActivity = DragDropMultipleObjectsComponent(onCompleted: nextStep, correctAssetLinks: actions[correctIndex].toLowerCase(), wrongAssetLinks: wrongActions, mainData: actions[correctIndex], onCorrectAction: triggerCorrectFlash, onIncorrectAction: triggerIncorrectFlash, directory: "assets/actions_images/", objectVariation: "Action",);
    }
