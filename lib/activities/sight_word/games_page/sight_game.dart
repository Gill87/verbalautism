...
// List
  ...
  void setTwoWrongNumbers(){
    ...
    wrongSightWords = [sightWords[randomNumber2].toLowerCase(), sightWords[randomNumber3].toLowerCase()];
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (totalSteps % 2 == 1 && totalSteps <= 10) {
      currentActivity = TapComponent(onCompleted: nextStep, assetLink: sightWords[randomNumber].toLowerCase(), mainData: sightWords[correctIndex], onCorrectAction: triggerCorrectFlash, totalSteps: totalSteps, directory: "assets/sight_word_images/", objectVariation: "Sight Word",);
    } 
    else if (totalSteps % 2 == 0 && totalSteps <= 10) {
      currentActivity = DragDropComponent(onCompleted: nextStep, assetLink: sightWords[randomNumber].toLowerCase(), mainData: sightWords[correctIndex], onCorrectAction: triggerCorrectFlash, totalSteps: totalSteps, directory: "assets/sight_word_images/", objectVariation: "Sight Word",);
    } 
    else if(totalSteps % 2 == 1 && totalSteps >= 10){
      currentActivity = TapMultipleObjectsComponent(onCompleted: nextStep, correctAssetLink: sightWords[correctIndex].toLowerCase(), wrongAssetLinks: wrongSightWords, mainData: sightWords[correctIndex], onCorrectAction: triggerCorrectFlash, onIncorrectAction: triggerIncorrectFlash, directory: "assets/sight_word_images/", objectVariation: "Sight Word",);
    }
    else {
      currentActivity = DragDropMultipleObjectsComponent(onCompleted: nextStep, correctAssetLinks: sightWords[correctIndex].toLowerCase(), wrongAssetLinks: wrongSightWords, mainData: sightWords[correctIndex], onCorrectAction: triggerCorrectFlash, onIncorrectAction: triggerIncorrectFlash, directory: "assets/sight_word_images/", objectVariation: "Sight Word",);
    }

    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Center(child: Text("Sight Words", style: GoogleFonts.ubuntu(fontSize:24, fontWeight: FontWeight.bold, color: Colors.white))),
